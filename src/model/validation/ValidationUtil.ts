...
import { ERROR_REQUIRED_FIELD, type ValidationResult } from "./Validation";
...
export class ValidationUtil {
  ...
  public static validateModel(modelClass: ModelDef, 
    ...
    if (this.debug) {
      logger?.log(`validateModel beforeCheck:`, model);
    }
    for (let f of Object.keys(modelClass.fields)) {
      ...
      if (option.autoCorrect) {
        ...
        fieldMeta.autoTrim && typeof v === 'string') {
          ...
          if (dayjs.isDayjs(v)) {
            model[f] = (<dayjs.Dayjs>v).format('YYYY-MM-DD');
            v = model[f];
          } else if (typeof v == 'object' && v?.getYear !== undefined) {
            model[f] = dayjs(v).format('YYYY-MM-DD');
            v = model[f];
          }
        }
      }
      ...
      if (!errors[f] && fieldMeta.rules) {
        for (let rule of fieldMeta.rules) {
          ...
          if (this.debug) {
            logger?.log(`validateModel field=[${f}], rule=[${rule.name}] result(failure)=`, ruleResult);
          }
          if (ruleResult) {
            errors[f] = ruleResult;
            break;
