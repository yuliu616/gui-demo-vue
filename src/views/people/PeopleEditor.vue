        ...
        >
          ...
          >
            <a-form-item :label="i18n.people.model.People['field.nickname']"
              :validate-status="errorFields['nickname']"
              :help="errorReasons['nickname']"
            >
            ...
            <a-form-item :label="i18n.people.model.People['field.gender']"
              :validate-status="errorFields['gender']"
              :help="errorReasons['gender']"
            >
            ...
            <a-form-item :label="i18n.people.model.People['field.dateOfBirth']"
              :validate-status="errorFields['dateOfBirth']"
              :help="errorReasons['dateOfBirth']"
            >
              <a-date-picker v-model:value="record.dateOfBirth"
        ...
        <a-card>
          ...
          >
            <a-form-item :label="i18n.people.model.People['field.firstName']"
              :validate-status="errorFields['firstName']"
              :help="errorReasons['firstName']"
            >
            ...
            <a-form-item :label="i18n.people.model.People['field.lastName']"
              :validate-status="errorFields['lastName']"
              :help="errorReasons['lastName']"
            >
            ...
            <a-form-item :label="i18n.people.model.People['field.heightInCm']"
              :validate-status="errorFields['heightInCm']"
              :help="errorReasons['heightInCm']"
            >
              <a-input-number v-model:value="record.heightInCm"
                class="my antd-input-number"
                allow-clear :readonly="!viewReady"
                :placeholder="i18n.people.model.People['field.heightInCm']"
                :min="1" :max="500" :precision="0"
              />
            </a-form-item>
...
import { PeopleMeta, type People } from '@/model/people/People';
...
import { PeopleUtil } from '@/util/people/PeopleUtil';
...
interface DictModel {
  ...
  GenderOptions: KeyValueAndLabel[];
}

export default {
  computed: {
    viewName(): string { return this.i18n.view['view.People Editor']},
    ...
    dict: function(): DictModel {
      return <DictModel>{
        EditMode: EditMode,
        GenderOptions: Object.values(Gender).map(code=>({
          value: code,
          label: this.i18n.t('people.word', 'word.enum.Gender.'+code),
        })),
      };
    },
  ...
  async mounted() {
    ...
    let afterEachHook = this.$router.afterEach((to, from, failure)=>{
      if (from.fullPath == to.fullPath) {
        // do nothing
      } else if (to.fullPath == '/people/creation') {
        if (debug) logger?.log('change to NEW mode.');
        this.onRouteChanged(EditMode.NEW);
      } else if (new RegExp('people/item/[0-9]*/edit').test(to.fullPath)) {
  ...
  methods: {
    ...
    validateInput(action: 'update' | 'create'): boolean {
      let result = ValidationUtil.validateModel(PeopleMeta, 
    ...
    onClosePressed(){
      if (this.viewTargetId === undefined) {
        ...
        this.$router.push(`/people/item/${this.viewTargetId}/view`);
      }
    },
    async onSavePressed(){
      ...
      let reloaded;
      try {
        reloaded = await this.iPeopleService.put_people_x(this.viewTargetId!, {
          body: this.record,
          consumer: this,
        });
      } catch (err) {
      ...
      await this.iMessageService.good(this, StringUtil.formatString(
        ...
        reloaded.version,
        PeopleUtil.stringPresentationFor(reloaded, this.i18n) ||
    ...
    async onCreationPressed(){
      ...
      let reloaded;
      try {
        reloaded = await this.iPeopleService.post_people({
          body: this.record,
          consumer: this,
        });
      } catch (err){
      ...
      this.$router.push(`/people/item/${reloaded.id}/view`);
    },
  },
};
</script>
